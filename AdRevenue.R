...
# Data found at http://gattonweb.uky.edu/sheather/book/data_sets.php in the AdRevenue.csv file

# Create a dataset of the advertising revenue data (some apostrophes removed in data in order to read in cleanly)
adRev <- read.csv(header = TRUE, text = '
...
Womans Day,"Lagardere, (Hachette Filipacchi Media U.S.)",237.004,4.027
...
Readers Digest,Readers Digest Association,291.034,10.094
...
Harpers Bazaar,"Hearst Corp., (Magazine Division)",93.416,0.726
...
Mens Health,Rodale,141.76,1.805
...
# Part a
# Fit the model: AdRevenue = beta0 + beta1 Circulation + epsilon
adRev_out <- lm(AdRevenue ~ Circulation, data = adRev)
summary(adRev_out)

# Find a 95% prediction interval for the AdRevenue with 0.5 (million) circulations
predict(adRev_out, newdata = data.frame(Circulation = 0.5), interval = "prediction")

# Find a 95% prediction interval for the AdRevenue with 20 (million) circulations
predict(adRev_out, newdata = data.frame(Circulation = 20), interval = "prediction")
