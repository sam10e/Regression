...
1.02 H VS2  HRD  8959
1.09 I VVS2 HRD  9107', header = FALSE, sep = "")

# Add header
names (diamonds) <- c("Carat", "Color", "Clarity", "Cert", "Price")

# Check to see that the data was read in correctly
str(diamonds)
tail(diamonds)
...
plot(Price~Carat, data = diamonds,
     main = "Price of Diamonds based on Carats")

# Create a natural log plot of Price and Carat
lnPrice <- log(diamonds$Price)
lnCarat <- log(diamonds$Carat)
plot(lnPrice~lnCarat, data = diamonds,
     main = "ln - Price of Diamonds based on Carats")

# Analysis
# Response Variable: Price
# Explanatory Variable: Carat

# Model: log(Price) = beta0 + beta1 log(Carat) + epsilon, epsilon~N(0, sigma2)
diamonds_out <- lm(log(Price)~log(Carat), data = diamonds)

# Table of estimates and std errors (and t-test)
summary(diamonds_out)

# On average, for every one unit increase in log(Carat), 
# we estimate an increase of 1.537 in log(Price).

# Without log (multiplicative, hence the percentage)
# For a 1% increase in Carat, we estimate an increase of 1.537% in Price on average.

# Carat has a statistically significant effect on Price (pvalue < 0.001)

# ANOVA Table
anova(diamonds_out)

# 95% confidence interval on beta1
confint(diamonds_out)

# For a 1% increase in Carat, we estimate an increase of 1.537% (95% CI: 1.501, 1.579) 
# in Price on average.

# Estimated Model: log(Price)(hat) = 9.128 + 1.537*log(Carat)

# Jewelry Store Manager wants to see how well the model is estimated
# Grpahic showing the uncertainty in estimating the model
library(ggplot2)
qplot(log(Carat), log(Price), data = diamonds,
      geom = "smooth", formula = y~x, method = "lm", se=TRUE,
      xlab = "Diamond Size (log Carat)",
      ylab = "Price (log Singapore $)")

# Predict the cost for a couple shopping for a one carat diamond
exp(predict(diamonds_out, newdata = data.frame(Carat = 1.0), interval = "prediction"))

# Graphic of uncertainty with predictions
plot.df <- cbind(diamonds, exp(predict(diamonds_out, interval = "prediction")))
...
summary(abs(diamonds$Price - exp(predict(diamonds_out))))